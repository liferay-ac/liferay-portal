...
const REMOTE_OPERATORS = new Set([
	...
	(CustomFunctionOperators as any).TagsFilter,
	(NotOperators as any).NotTagsFilter
]);
...
export default function CriteriaSidebar({
	...
	const apiMethod = isTagsSection ? API.tags.search : API.vocabularies.search;
	...
	useEffect(() => {
		...
		remoteCriteria.forEach(({id, name, isTag}) => {
			if (isTag) {
