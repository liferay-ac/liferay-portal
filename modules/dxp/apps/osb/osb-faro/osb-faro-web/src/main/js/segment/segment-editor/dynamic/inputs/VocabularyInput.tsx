...
export const OCCURRENCE_OPTIONS = [
	{label: Liferay.Language.get('any-fragment'), value: 'any'},
	{
...
function buildValue(
	...
	const isAny = occurrenceOperator === 'any';
	...
	return createCustomValueMap([
		{key: 'operator', value: isAny ? null : occurrenceOperator},
		{key: 'value', value: isAny ? null : occurrenceCount},
...
export default function VocabularyInput({
	...
	const [occurrenceOperator, setOccurrenceOperator] = useState<React.Key>(
		rawOperator === null ? 'any' : (rawOperator ?? RelationalOperators.GE)
	);
	...
	const isOccurrenceValid = (
		operator: React.Key,
		count: number | string
	): boolean => operator === 'any' || isValidOccurrenceCount(count);
	...
	return (
		<div className='criteria-statement'>
			...
			<Form.Group autoFit>
				...
				{occurrenceOperator !== 'any' && (
					<>
						<Form.GroupItem
							className={getCN({
								'has-error':
									!occurrenceValid && occurrenceTouched
							})}
							shrink
						>
							<Input
								...
								onBlur={({
									...
									const valid =
										isValidOccurrenceCount(inputVal);
								...
								onChange={({
									...
									const valid =
										isValidOccurrenceCount(numberVal);
