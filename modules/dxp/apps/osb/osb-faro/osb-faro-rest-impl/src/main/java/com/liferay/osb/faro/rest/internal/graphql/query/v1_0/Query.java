...
public class Query {
	 ...
	 */
	@GraphQLField(
		description = "List analytics asset summaries (page, blog, document, form, journal, object-entry) ranked by the requested sort metric, returning download/impression/read/view counts plus their period-over-period trend percentages, optionally narrowed to a single channel (also know as property) or to a date range. Use this to answer 'what content is performing best' and to pick assets for deeper drill-down via `getSitePagesPage`."
	)
	 ...
	 */
	@GraphQLField(
		description = "List tracked analytics events (page views, custom events, form submissions, etc.) for a specific channel (also known as property), optionally narrowed to a date range. Use this to inspect raw activity for a channel; for aggregated metrics across events, prefer `getSiteAssetSummariesPage` or `getSitePagesPage`."
	)
	 ...
	 */
	@GraphQLField(
		description = "List analytics metrics for tracked pages on the site, ranked by views or another metric, optionally narrowed to a single channel (also known as property) or data source. Use this for 'top pages' style queries. Returns flattened view, visitor, bounce, exit, and access-path metrics for each page; for cross-asset comparisons that include non-pages, use `getSiteAssetSummariesPage`."
	)
...
// LIFERAY-REST-BUILDER-HASH:1338677088