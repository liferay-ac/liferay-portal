...
import com.liferay.segments.asah.rest.client.dto.v1_0.Segment;
...
import com.liferay.segments.asah.rest.client.resource.v1_0.SegmentResource;
...
public abstract class BaseSegmentResourceTestCase {
	...
	public void testEscapeRegexInStringFields() throws Exception {
		...
		Assert.assertEquals(regex, segment.getId());
		Assert.assertEquals(regex, segment.getName());
	}

	@Test
	public void testPostSegment() throws Exception {
		Segment randomSegment = randomSegment();

		Segment postSegment = testPostSegment_addSegment(randomSegment);

		assertEquals(randomSegment, postSegment);
		assertValid(postSegment);
	}

	protected Segment testPostSegment_addSegment(Segment segment)
		throws Exception {
	...
	@Test
	public void testGraphQLPostSegment() throws Exception {
		Segment randomSegment = randomSegment();

		Segment segment = testGraphQLSegment_addSegment(
			testGraphQLPostSegment_getSegmentsEntryKey(), randomSegment);

		Assert.assertTrue(equals(randomSegment, segment));
	}

	protected String testGraphQLPostSegment_getSegmentsEntryKey()
		throws Exception {
	...
	protected Segment testGraphQLSegment_addSegment() throws Exception {
		return testGraphQLSegment_addSegment(
			testGraphQLSegment_getSegmentsEntryKey(), randomSegment());
	}

	protected String testGraphQLSegment_getSegmentsEntryKey() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}

	protected Segment testGraphQLSegment_addSegment(
			String segmentsEntryKey, Segment segment)
		throws Exception {

		JSONDeserializer<Segment> jsonDeserializer =
			JSONFactoryUtil.createJSONDeserializer();

		StringBuilder sb = new StringBuilder("{");

		for (java.lang.reflect.Field field : getDeclaredFields(Segment.class)) {
			if (getGraphQLValue(field.get(segment)) != null) {
				if (sb.length() > 1) {
					sb.append(", ");
				}
				...
				sb.append(": ");
				sb.append(getGraphQLValue(field.get(segment)));
			}
		}
						...
						new HashMap<String, Object>() {
							{
								put(
									"segmentsEntryKey",
									"\"" + segmentsEntryKey + "\"");
								put("segment", sb.toString());
							}
						},
						graphQLFields)),
				"JSONObject/data", "JSONObject/createSegment"),
			Segment.class);
	}
	...
	protected void assertValid(Segment segment) throws Exception {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("memberships", additionalAssertFieldName)) {
				if (segment.getMemberships() == null) {
					valid = false;
				}
	...
	protected List<GraphQLField> getGraphQLFields() throws Exception {
		...
		for (java.lang.reflect.Field field :
				getDeclaredFields(
					com.liferay.segments.asah.rest.dto.v1_0.Segment.class)) {
	...
	protected boolean equals(Segment segment1, Segment segment2) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("memberships", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						segment1.getMemberships(), segment2.getMemberships())) {
	...
	@Inject
	private com.liferay.segments.asah.rest.resource.v1_0.SegmentResource
		_segmentResource;
