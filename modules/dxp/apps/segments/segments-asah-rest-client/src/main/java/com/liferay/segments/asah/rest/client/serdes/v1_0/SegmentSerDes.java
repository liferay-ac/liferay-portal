...
public class SegmentSerDes {
	...
	public static String toJSON(Segment segment) {
		...
		if (segment.getId() != null) {
			...
			sb.append(_escape(segment.getId()));
		...
		if (segment.getMemberships() != null) {
			...
			for (int i = 0; i < segment.getMemberships().length; i++) {
				sb.append(String.valueOf(segment.getMemberships()[i]));

				if ((i + 1) < segment.getMemberships().length) {
					sb.append(", ");
				}
			}
	...
	public static Map<String, String> toMap(Segment segment) {
		...
		if (segment.getMemberships() == null) {
			...
			map.put("memberships", String.valueOf(segment.getMemberships()));
		}
	...
	public static class SegmentJSONParser extends BaseJSONParser<Segment> {
		...
		protected void setField(
			...
			if (Objects.equals(jsonParserFieldName, "active")) {
				...
				if (jsonParserFieldValue != null) {
					segment.setId((String)jsonParserFieldValue);
				}
			}
			else if (Objects.equals(jsonParserFieldName, "memberships")) {
				if (jsonParserFieldValue != null) {
					...
					segment.setMemberships(membershipsArray);
				}
			}
			else if (Objects.equals(jsonParserFieldName, "name")) {
				if (jsonParserFieldValue != null) {
