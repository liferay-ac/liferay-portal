...
test(
	'Documents visitor behavior card shows expected amount of views',
	async ({apiHelpers, page}) => {
		...
		await test.step('Go to Documents and Media session', async () => {
			await page.locator('.navbar-collapse').getByText('Documents and Media').click();
		});
		...
		await test.step('Assert the document is appearing at the list', async () => {
			const documentTitles = await page.locator('.documents-and-media-root .table-title').all();
		...
		await test.step('Assert the document list', async () => {
			...
			documentTitles = await page.locator('.documents-and-media-root .table-title').all();

			expect(documentTitles.length).toBe(1);

			expect(await page.locator('.documents-and-media-root .table-title').getByText('My Document 1')).toBeVisible();
		});
		...
		await test.step('Go into document and check Visitors Behavior metrics', async () => {
			...
			const metricTabs = await page.locator('.analytics-metrics-tabs .card-tab').all();

			const downloadsMetricTab = metricTabs[0];

			expect(await downloadsMetricTab.locator('.metric-value').textContent()).toBe("3");

			const impressionsMetricTab = metricTabs[1];
			
			expect(await impressionsMetricTab.locator('.metric-value').textContent()).toBe("2");
		});
	}
);
