...
test(
	'View all blogs in the property in assets',
	async ({apiHelpers, page}) => {
		...
		await test.step('Assert the blog list', async () => {
			...
			expect(await page.locator('.blogs-root .table-title').getByText('My Blog 2')).toBeVisible();
			...
			expect(await page.locator('.blogs-root .table-title').getByText('My Blog 1')).toBeVisible();
		});
	}
);
