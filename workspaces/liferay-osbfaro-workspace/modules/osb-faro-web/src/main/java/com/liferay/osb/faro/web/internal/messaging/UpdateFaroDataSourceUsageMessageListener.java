...
public class UpdateFaroDataSourceUsageMessageListener
		...
		for (FaroProject faroProject :
			...
			try {
				_addOrUpdateFaroDataSourceUsages(
					date, faroProject, dataSourceUsageMetricsMap);
			}
			catch (Exception exception) {
				_log.error(
	...
	private void _addOrUpdateFaroDataSourceUsages(
		...
		Map<String, List<DataSourceUsageMetric>> dataSourceUsageMetrics =
			dataSourceUsageMetricsMap.get(faroProject.getServerLocation());

		if (dataSourceUsageMetrics == null) {
			dataSourceUsageMetrics = new HashMap<>();
			...
			for (DataSourceUsageMetric dataSourceUsageMetric :
				...
				List<DataSourceUsageMetric> projectDataSourceUsageMetrics =
					dataSourceUsageMetrics.computeIfAbsent(
						dataSourceUsageMetric.getProjectId(),
						projectId -> new ArrayList<>());

				projectDataSourceUsageMetrics.add(dataSourceUsageMetric);
			}

			dataSourceUsageMetricsMap.put(
				faroProject.getServerLocation(), dataSourceUsageMetrics);
		}

		List<DataSourceUsageMetric> dataSourceUsageMetricsForProject =
			dataSourceUsageMetrics.get(faroProject.getProjectId());

		if (dataSourceUsageMetricsForProject == null) {
			return;
		}

		for (DataSourceUsageMetric dataSourceUsageMetric :
				dataSourceUsageMetricsForProject) {

			_faroDataSourceUsageLocalService.addOrUpdateFaroDataSourceUsage(
				dataSourceUsageMetric.getBillableEventsCount(),
				faroProject.getCompanyId(),
				...
				dataSourceUsageMetric.getKnownIndividualsCount(), date, 0);
		}
	}
